...
const actions = {
  ...
  async runAllIdle() {
    ...
    // Fire concurrently — backend handles its own queuing
    await Promise.allSettled(
      idle.map(s => actions.runSubtopic(s.subtopic_id))
    );
  },
  ...
  async _tick() {
    ...
    await Promise.allSettled(
      running.map(async s => {
        ...
        // When a run completes, auto-load the draft
        if (prev === "running" && rs.status === "done") {
          await actions._loadDraft(s.subtopic_id);
          // Refresh active draft if this is the active subtopic
